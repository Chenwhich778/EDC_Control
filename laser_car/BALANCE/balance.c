...
{ 
    ...
    // 运动控制参数
    #define FIELD_SIZE     3500    // 场地尺寸3.5m = 3500mm
    #define CRUISE_SPEED  2     // 巡航速度500mm/s (0.5m/s)
    #define STAGE_TIME    2000    // 每段移动时间(ms): 3500/0.5=7000ms
    
    static PathStage stage = STAGE_RIGHT;
    static uint8_t lapCount = 0;
    static uint32_t stageStartTime = 0;
    static uint8_t autoNavigation = 1;  // 启用自动导航
    ...
    {	
        ...
        if (autoNavigation && Check == 0) {
            ...
            switch (stage) {
                case STAGE_RIGHT:  // 向右移动
                    Move_X = 0;
                    Move_Y = CRUISE_SPEED;  // 正Y方向为右移
                    Move_Z = 0;
                ...
                case STAGE_LEFT:  // 向左移动
                    Move_X = 0;
                    Move_Y = -CRUISE_SPEED;  // 负Y方向为左移
                    Move_Z = 0;
                ...
                case STAGE_DOWN:  // 向下移动
                    Move_X = -CRUISE_SPEED;  // 负X方向为后退
                    Move_Y = 0;
                    Move_Z = 0;
                    ...
                    if (elapsed >= STAGE_TIME) {
                        if (++lapCount >= 2) {
                            ...
                            autoNavigation = 0;  // 完成导航
                        } else {
                            stage = STAGE_RIGHT; // 开始下一圈
                        }
                        stageStartTime = currentTime;
                    }
                    break;
                    
                case STAGE_COMPLETE:  // 导航完成
                    Move_X = 0;
                    Move_Y = 0;
                    Move_Z = 0;
