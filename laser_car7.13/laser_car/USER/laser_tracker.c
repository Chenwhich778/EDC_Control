...
int servo_y = 2000;  // 垂直舵机初始值 (2000-2100)
int last_obj_x = 343;  // 上一帧物体x坐标
int last_obj_y = 193;  // 上一帧物体y坐标
int lock_count = 0;    // 锁定计数器
int move_counter = 0;  // 移动间隔计数器
int consecutive_steps = 0; // 连续移动计数

int scan_mode = 0;            // 扫描模式标志
int scan_direction = 1;       // 扫描方向 (1:向右, -1:向左)
const int SCAN_MIN = 1780;    // 扫描左边界
const int SCAN_MAX = 2150;    // 扫描右边界
const float SCAN_STEP = 0.25;      // 扫描步长
const int SCAN_VELOCITY = 400;     // 扫描移动延迟(ms)
float addition = 2000;
...
FineServo fine_servo_x = {1980, 0};
FineServo fine_servo_y = {2050, 0};

void scan_for_object() {
    // 更新扫描位置
    addition += scan_direction*SCAN_STEP;
    servo_x = (int)addition;
    
    // 检查边界并反转方向
    if (servo_x >= SCAN_MAX) {
        servo_x = SCAN_MAX;
        scan_direction = -1;  // 向右到头，改为向左
    } else if (servo_x <= SCAN_MIN) {
        servo_x = SCAN_MIN;
        scan_direction = 1;   // 向左到头，改为向右
    }
    
    // 发送扫描指令
    Servo_SetPosition(1, servo_x, SCAN_VELOCITY);
    
    // 垂直方向保持不动
    Servo_SetPosition(2, servo_y, SCAN_VELOCITY);
}

void control_camera(int obj_x, int obj_y) {
    // 目标固定点坐标（相机中心）
    const int TARGET_X = 343;
    const int TARGET_Y = 193;
    ...
    const float FINE_STEP_SIZE = 0.5f;  // 精细步长（从0.25减小到0.2）
    float fine_step_x = 0;
    float fine_step_y = 0;
    ...
    // 默认锁定阈值
    int LOCK_THRESHOLD_X = 15;
    int LOCK_THRESHOLD_Y = 15;
    // // 当识别到的尺寸小于28时，阈值放宽到6
    // if (info.size < 28.0f) {
    //     LOCK_THRESHOLD_X = 7;
    //     LOCK_THRESHOLD_Y = 7;
    ...
    if (fine_step_x != 0) {
        ...
        if (fabs(fine_servo_x.fine) >= 1.0f) {
            ...
            fine_servo_x.base = constrain(fine_servo_x.base + actual_step, 1500, 4000);
    ...
    if (fine_step_y != 0) {
        ...
        if (fabs(fine_servo_y.fine) >= 1.0f) {
            ...
            fine_servo_y.base = constrain(fine_servo_y.base + actual_step, 1900, 2400);
