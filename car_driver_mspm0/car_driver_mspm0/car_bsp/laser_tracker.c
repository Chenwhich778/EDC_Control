...
// 水平方向PID参数（较大系数）
#define KP_X      0.004f  // 比例系数（较大值）
#define KI_X      0.000f  // 积分系数
#define KD_X      0.0002f // 微分系数

// 垂直方向PID参数（较小系数）
#define KP_Y      0.003f  // 比例系数（较小值）
#define KI_Y      0.000f
#define KD_Y      0.000f

// 系统全局变量
SystemMode system_mode = MODE_RADAR_SCANNING;   // 当前系统模式
int radar_servo_position = 2048;               // 雷达舵机初始位置（中位）
...
FineServo fine_servo_x = {1900, 0};  // 水平精细控制
FineServo fine_servo_y = {2200, 0};  // 垂直精细控制
...
void control_camera(int obj_x, int obj_y) {
    ...
    if (fine_step_x != 0) {
        ...
        if (fabs(fine_servo_x.fine) >= 1.0f) {
            ...
            fine_servo_x.base = constrain(fine_servo_x.base + actual_step, target_position - 200, target_position + 200);
    ...
    if (fine_step_y != 0) {
        ...
        if (fabs(fine_servo_y.fine) >= 1.0f) {
            ...
            fine_servo_y.base = constrain(fine_servo_y.base + actual_step, 2100, 2260);
