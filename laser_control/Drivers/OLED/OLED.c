...
//	GPIO_WriteBit(GPIOB, GPIO_Pin_12, (BitAction)BitValue);
	HAL_GPIO_WritePin(OLED_SCK_GPIO_Port,OLED_SCK_Pin,(GPIO_PinState)BitValue);
}
...
void OLED_W_RES(uint8_t BitValue)
{
	...
	HAL_GPIO_WritePin(OLED_RST_GPIO_Port, OLED_RST_Pin, (GPIO_PinState)BitValue);
}
...
void OLED_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
	...
	uint16_t XOffset = 0;
//	uint16_t pIndex;2025/5/31手动屏蔽hyf
	...
	while (String[i] != '\0')	//遍历字符串
	{
		...
		else					//否则，即多字节字符
		{
			...
			if (FontSize == OLED_6X8)	//给定字体为6*8点阵
			{
				/*空间不足，此位置显示'?'*/
