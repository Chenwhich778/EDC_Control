...
const float forbidden_radius = 0.25f; // 20cm

// 预设敌人绝对坐标 (根据实际需求设置)
float PRESET_ENEMY_X = 2.5f;
...
uint32_t LineDiffParam = 49; // 纠偏系数  everloss......change
...
static float wheelCoefficient(uint32_t diffparam, uint8_t isLeftWheel)
{
	...
	{
		if (diffparam >= 50)
			return 1.0f + 0.005f * (diffparam - 50);
	}
	else // 右轮纠偏,50~0对应1.0~1.2倍的纠偏系数
	{
		if (diffparam <= 50)
			return 1.0f + 0.005f * (50 - diffparam);
	}
...
void Balance_task(void *pvParameters)
{
	...
	typedef enum
	{
		STAGE_FORWARD,	 // 向前
		STAGE_LEFT,		 // 左移
		STAGE_SIDE_MOVE, // 左右移动阶段
		STAGE_SIDE_STOP, // 左右移动后的停止阶段
		STAGE_COMPLETE	 // 完成
	} MoveStage;

#define CRUISE_SPEED 0.2
#define FORWARD_TIME 2700
#define LEFT_TIME 3000
#define MOVE_TIME 4000 // 移动时间
int STOP_TIME = 3500 ;// 停止时间
#define SIDE_REPEAT 100  // 左右共20次

	static MoveStage stage = STAGE_FORWARD;
	static int side_dir = 1; // 1右 -1左
	static uint8_t side_count = 0;
	...
	while (1)
	{
		...
		if (SysVal.Time_count < 3000)
			SysVal.Time_count++;
		Buzzer_count1++;
        if(X_enemy*X_enemy + Y_enemy*Y_enemy > 4.0f&&X_enemy == 0)
		{
			STOP_TIME = 0;
		}
		else
		...
		{
			uint32_t currentTime = getSysTickCnt() * portTICK_PERIOD_MS;
			if (stageStartTime == 0)
			...
			case STAGE_LEFT:
				Move_X = 0;
				Move_Y = 4*CRUISE_SPEED;
				Move_Z = 0;
				if (elapsed >= LEFT_TIME)
				{
					stage = STAGE_SIDE_MOVE;
					stageStartTime = currentTime;
					side_dir = 1; // 第一次重复为向右
				}
				break;

			case STAGE_SIDE_MOVE:
				Move_X = 0;
				Move_Y = 1.5*CRUISE_SPEED * side_dir;
				Move_Z = 0;
				if (elapsed >= MOVE_TIME)
				{
					stage = STAGE_SIDE_STOP;
					stageStartTime = currentTime;
				}
				break;

			case STAGE_SIDE_STOP:
				Move_X = 0;
				Move_Y = 0;
				Move_Z = 0; // 停止移动
				if (elapsed >= STOP_TIME)
				{
					...
					else
					{
						// 回到移动阶段
						stage = STAGE_SIDE_MOVE;
						stageStartTime = getSysTickCnt() * portTICK_PERIOD_MS;
					}
				}
				break;
